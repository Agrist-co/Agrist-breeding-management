...
with tab1:
    ...
    _left_col, _right_col = st.columns([3, 1])
    with _left_col:
        with c1:
    ...
    with _right_col:
        st.markdown("**◆ タンク残量**")
        # 農場内の全育成中鶏舎のタンク残量を表示
        _r_farm_ln_ids = {ln["lot_number_id"] for ln in lot_numbers if ln["farm_id"] == sel_farm_id}
        _r_fhs = [fh for fh in flock_houses
                  if fh["lot_number_id"] in _r_farm_ln_ids and fh.get("status") == "育成中"]

        for _r_fh in _r_fhs:
            _r_house = next((h for h in houses if h["house_id"] == _r_fh["house_id"]), {})
            _r_house_name  = _r_house.get("house_name", "")
            _r_tank_no     = _r_house.get("tank_number", "-")
            _r_tank_cap    = float(_r_house.get("tank_capacity") or 5000)

            # 最新の予測残量をfeed_order_detailsから取得
            _r_details = supabase.table("feed_order_details") \
                .select("pred_tank,delivery_date") \
                .eq("flock_house_id", _r_fh["flock_house_id"]) \
                .is_("order_id", "null") \
                .order("delivery_date").execute().data
            # 今日以降で最も近い予測残量
            _r_today = date.today()
            _r_pred  = None
            for _rd in _r_details:
                if _rd.get("delivery_date") and date.fromisoformat(_rd["delivery_date"]) >= _r_today:
                    _r_pred = _rd.get("pred_tank")
                    break
            if _r_pred is None and _r_details:
                _r_pred = _r_details[-1].get("pred_tank") or 0
            _r_pred = float(_r_pred or 0)

            # 残量比率（0〜1）
            _r_ratio = max(0.0, min(1.0, _r_pred / _r_tank_cap)) if _r_tank_cap > 0 else 0.0
            # 残量に応じた色
            _r_color = "#e74c3c" if _r_ratio < 0.15 else "#f39c12" if _r_ratio < 0.30 else "#2ecc71"
            _r_pct   = int(_r_ratio * 100)
            _r_fill_h = int(120 * _r_ratio)  # 最大120px

            _tank_svg = f"""
<div style="text-align:center;margin-bottom:12px;">
  <div style="font-size:0.75rem;font-weight:600;color:#444;margin-bottom:2px;">
    {_r_house_name}（タンク{_r_tank_no}）
  </div>
  <svg width="60" height="150" viewBox="0 0 60 150" xmlns="http://www.w3.org/2000/svg">
    <!-- タンク外枠 -->
    <rect x="5" y="10" width="50" height="130" rx="6" ry="6"
          fill="#f0f0f0" stroke="#999" stroke-width="1.5"/>
    <!-- 残量（下から） -->
    <rect x="6" y="{140 - _r_fill_h}" width="48" height="{_r_fill_h}" rx="4" ry="4"
          fill="{_r_color}" opacity="0.85"/>
    <!-- メモリ線 -->
    <line x1="5" y1="53" x2="55" y2="53" stroke="#ccc" stroke-width="0.8" stroke-dasharray="3,2"/>
    <line x1="5" y1="97" x2="55" y2="97" stroke="#ccc" stroke-width="0.8" stroke-dasharray="3,2"/>
    <!-- パーセント表示 -->
    <text x="30" y="80" text-anchor="middle" font-size="13" font-weight="bold"
          fill="{_r_color if _r_ratio < 0.3 else '#333'}">{_r_pct}%</text>
  </svg>
  <div style="font-size:0.72rem;color:#555;">{_r_pred:,.0f} kg</div>
  <div style="font-size:0.68rem;color:#888;">/ {_r_tank_cap:,.0f} kg</div>
</div>"""
            st.markdown(_tank_svg, unsafe_allow_html=True)
