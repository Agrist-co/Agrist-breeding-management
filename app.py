...
with main_tabs[0]:
    ...
    # 直前の保存・読込操作の結果メッセージ（st.rerun()後も見えるようにここで表示する）
    if "flash_message" in st.session_state:
        _msg_type, _msg_text = st.session_state.pop("flash_message")
        if _msg_type == "success":
            st.success(_msg_text)
        else:
            st.error(_msg_text)

    # 「過去データ読込」ボタンが押された直後は、読み込んだ値をウィジェットの初期値として使う
    _d = st.session_state.get("loaded_params", {})

    col1, col2, col3 = st.columns(3)
    with col1:
        farm_name = st.text_input("農場名:", _d.get("farm_name", "上川西農場"), key="farm_name")
        start_date = st.date_input(
            "入雛日:",
            datetime.strptime(_d["start_date"], "%Y-%m-%d").date() if "start_date" in _d else date(2026, 6, 9),
            key="start_date"
        )
        birds = st.number_input("入雛羽数(羽):", value=_d.get("birds", 6600), step=100, key="birds")
        shipping_age = st.number_input("出荷日齢:", value=_d.get("shipping_age", 46), max_value=56, key="shipping_age")
    with col2:
        house_no = st.text_input("鶏舎No./名:", _d.get("house_no", "A棟"), key="house_no")
        tank_cap = st.number_input("タンク容量(kg):", value=_d.get("tank_cap", 7000), step=500, key="tank_cap")
        min_alert = st.number_input("最低残量アラート(kg):", value=_d.get("min_alert", 500), step=100, key="min_alert")
        first_qty = st.number_input("初回納品量(kg):", value=_d.get("first_qty", 5000), step=500, key="first_qty")
    with col3:
        tank_no = st.text_input("タンクNo.:", _d.get("tank_no", "No.1"), key="tank_no")
        ...
        if st.button("📂 選択したデータを読込", type="secondary"):
            if sel_farm != "(保存データなし)" and sel_date != "—":
                try:
                    ...
                    st.session_state.current_records = {int(k): v for k, v in loaded["records"].items()}
                    st.session_state.current_adjustments = {int(k): v for k, v in loaded.get("adjustments", {}).items()}
                    # 入力欄（農場名・入雛日など）にも反映させるため、次回描画時の初期値として保存し再実行する
                    st.session_state.loaded_params = {
                        ...
                        "start_date": loaded.get("start_date", sel_date),
                    ...
                    # ウィジェットは既にインスタンス化されているため直接は書き換えず、
